...
| 32 | Drop-in search + ingest-ckan fixes: Fixed DropInFilterPanel search gate that blocked first click when default programs were pre-selected (leisure-all, leisure-adult); updated condition to allow search when `selectedPrograms.length > 0`. Added comprehensive error handling in ingest-ckan edge function (safe destructuring, env var validation, try/catch at top level); verified function successfully ingests data (31,389 dropins from CKAN on May 4). Confirmed cron schedule active in config.toml (`0 7 * * 0` = Sunday 7am UTC). Drop-in search now fully functional. |
| 33 | Drop-in data refresh + UX: Investigated "drop-in search empty" report — found CKAN contains data through July 23, but our database was stale (only through June 11). Root cause: scheduled ingest-ckan hadn't run since June 11 (cron may have been skipped or failed silently). Manually triggered ingest-ckan via HTTP POST, loaded 30,270 dropins successfully. Improved empty state messaging in DropInResultsTable to explain data availability. June 12+ dates now searchable. **Note: monitor if cron continues to run reliably; consider dashboard for sync health.** |
